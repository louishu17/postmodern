...
//            if(rc.getRoundNum() > 5) comm.activateDanger();
        ...
        if(rc.getRoundNum() > 1200 && (rc.getResourceAmount(ResourceType.ADAMANTIUM) < 80 || rc.getResourceAmount(ResourceType.MANA) < 120)){
            return;
        }
...
//        carrierScore = comm.getBuildingScore(RobotType.CARRIER);
    ...
    boolean tryBuildAnchor(){
        try {
            if (rc.getRoundNum() > 400 && rc.getResourceAmount(ResourceType.ADAMANTIUM) >= 80 && rc.getResourceAmount(ResourceType.MANA) >= 80) {
